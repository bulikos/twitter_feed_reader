...
class DetailParser:
    ...
    def parse(self, data: dict[str, Any], focal_id: str) -> list[Tweet]:
        ...
        try:
            ...
            logger.debug("Starting DetailParser.parse | Instructions found: %d", len(instructions))
            ...
            for instruction in instructions:
                ...
                if inst_type == "TimelineAddEntries":
                    ...
                    logger.debug("Processing TimelineAddEntries | Count: %d", len(entries))
                elif inst_type == "TimelineAddToModule":
                    entries = instruction["moduleItems"]
                    logger.debug("Processing TimelineAddToModule | Count: %d", len(entries))
                else:
                    logger.debug("Skipping instruction type: %s", inst_type)

                for entry in entries:
                    self.stats.total_entries += 1
                    # entryId is often helpful for debugging specific items
                    eid = entry.get("entryId", "unknown")
                    logger.debug("Processing entry: %s", eid)

                    new_tweets = self._parse_entry(entry, focal_id)
                    if new_tweets:
                        logger.debug("Extracted %d tweets from entry %s", len(new_tweets), eid)
                    tweets.extend(new_tweets)

        except Exception as e:
            logger.error("Error parsing detail: %s", e, exc_info=True)
    ...
    def _parse_entry(self, entry: dict[str, Any], focal_id: str) -> list[Tweet]:
        ...
        if "item" in entry:
            ...
            if item_type == "TimelineTweet":
                ...
                logger.debug("  -> Skipping module item type: %s", item_type)
        ...
        # Structure: entry -> content -> ...
            ...
            if entry_type == "TimelineTimelineModule":
                ...
                logger.debug("  -> Found TimelineTimelineModule | Items: %d", len(items))
            ...
            # Sub-case 2.2: Single Tweet item
                ...
                logger.debug("  -> Unhandled entry content type: %s", entry_type)
