...
class ItemParser:
    ...
    def parse_timestamp(self, date_str: str | None) -> int:
        ...
        try:
            ...
            logger.debug("Failed to parse timestamp '%s': %s", date_str, e)
            return 0
    ...
    def extract_media(self, legacy: dict[str, Any]) -> list[str]:
        ...
        if media_urls:
            logger.debug("Extracted %d media URLs", len(media_urls))
        return media_urls
    ...
    def extract_text(self, result: dict[str, Any], legacy: dict[str, Any]) -> tuple[str, bool]:
        ...
        if "article" in result:
            if "article_results" in result["article"]:
                try:
                    ...
                    logger.debug("Failed to extract Article content: %s", e)
            else:
        ...
        if "note_tweet" in result:
            try:
                ...
                logger.debug("Failed to extract Note Tweet content: %s", e)
    ...
    def parse_tweet_result(self, result: dict[str, Any], source: str = "timeline") -> list[Tweet]:
        ...
        if not legacy:
            ...
            if typename == "TweetUnavailable":
                logger.debug("Skipping TweetUnavailable (reason: %s)", result.get("reason", "unknown"))
                return collected_tweets
            elif typename == "TweetTombstone":
                logger.debug("Skipping TweetTombstone")
                return collected_tweets

            logger.debug("Skipping tweet with no legacy data (typename: %s)", typename)
            return collected_tweets

        tid = legacy.get("id_str")
        logger.debug("Parsing tweet %s from source %s", tid, source)
        ...
        if core:
            ...
            logger.debug("No 'core' data found for tweet %s", tid)

        if user_handle == "unknown":
            logger.warning("Failed to extract user handle for tweet %s (user_id: %s)", tid, user_id)
        ...
        if "retweeted_status_result" in legacy:
            ...
            if rt_res and rt_res.get("__typename") == "Tweet":
                ...
                logger.debug("Recursing into retweet content for tweet %s", tid)
                ...
                if rt_tweets:
                    ...
                    logger.debug("Retweet recursion returned no tweets for %s", tid)
            else:
                logger.debug("Retweet status result missing or not 'Tweet' for %s", tid)
        ...
        if "quoted_status_result" in result:
            ...
            if quoted_res and quoted_res.get("__typename") == "Tweet":
                ...
                logger.debug("Recursing into quoted tweet content for tweet %s", tid)
                quoted_tweets = self.parse_tweet_result(quoted_res, source="quote")
                collected_tweets.extend(quoted_tweets)
            else:
                logger.debug("Quoted status result missing or not 'Tweet' for %s", tid)
